...
def _extract_features(df: pd.DataFrame) -> Tuple[np.ndarray, np.ndarray]:
    ...
    feature_order = [
        ...
        'order_flow_score', 'order_flow_flag', 'cvd', 'cvd_change', 'cvd_divergence',
        'cvd_absorption', 'cvd_accumulation', 'taker_buy_ratio',
    ...
    for _, row in df.iterrows():
        try:
            ...
            base_features = {
                ...
                'cvd_divergence': row.get('cvd_divergence', 0.0),
                'cvd_absorption': row.get('cvd_absorption', 0.0),
                'cvd_accumulation': row.get('cvd_accumulation', 0.0),
...
def train_model(iterations: int = 200, learning_rate: float = 0.1) -> Optional[TrainingDiagnostics]:
    ...
    if SKLEARN_AVAILABLE:
        ...
        feature_names = [
            ...
            'cvd_divergence', 'cvd_absorption', 'cvd_accumulation',
