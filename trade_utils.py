...
try:
    ...
    class ADXIndicator:
        ...
        def adx(self) -> pd.Series:
            ...
            plus_di = 100 * (
                pd.Series(plus_dm).rolling(self.window).mean()
                / (atr + 1e-9)
            )
            minus_di = 100 * (
                pd.Series(minus_dm).rolling(self.window).mean()
                / (atr + 1e-9)
            )
            dx = (
    ...
    class KeltnerChannel:
        ...
        def keltner_channel_hband(self) -> pd.Series:
            ...
            atr = AverageTrueRange(
                self.high, self.low, self.close, self.window
            ).average_true_range()
            return ema + (atr * 2)
...
try:
    ...
    class Client:  # type: ignore
        def __init__(self, *args, **kwargs):
            raise ImportError(
                "Binance client is not installed. Please install 'python-binance' to fetch price data."
            )
...
try:
    ...
    def is_price_near_zone(
        price: float, zones: dict, zone_type: str, tolerance: float
    ) -> bool:  # type: ignore
...
try:
    ...
    def recall_pattern_confidence(
        symbol: str, pattern_name: str
    ) -> float:  # type: ignore
...
SYMBOL_SCORES_FILE = os.path.join(
    os.path.dirname(__file__), "symbol_scores.json"
)
    ...
    try:
        if ZoneInfo is not None:
            now = (
                ...
                .astimezone(ZoneInfo("Asia/Karachi"))
            )
            hour = now.hour
        else:
...
def calculate_indicators(df: pd.DataFrame) -> pd.DataFrame:
    ...
    try:
        ...
        df["dema_20"] = DEMAIndicator(
            df["close"], window=20
        ).dema_indicator()
        df["tema_20"] = TEMAIndicator(
            df["close"], window=20
        ).tema_indicator()
        ...
        print(
            f"[INDICATOR] Warning: failed to compute some indicators: {e}"
        )
    return df
...
def get_top_symbols(limit: int = 30) -> list:
    ...
    if client is None:
        ...
        print(
            "⚠️ Binance client unavailable; get_top_symbols returning empty list."
        )
        return []
    ...
    symbols = [
        x["symbol"]
        for x in sorted_tickers
        if x["symbol"].endswith("USDT")
        and not x["symbol"].endswith("BUSD")
    ]
...
def evaluate_signal(
    ...
    try:
        if (
            price_data is None
            or price_data.empty
            or len(price_data) < 40
        ):
            print(
                f"[DEBUG] Skipping {symbol}: insufficient price data."
            )
        ...
        vwma = (
            vwma_calc.volume_weighted_average_price()
        )
        bb_upper = bb.bollinger_hband()
        bb_lower = bb.bollinger_lband()
        # New indicators
        dema_short = DEMAIndicator(
            close, window=20
        ).dema_indicator()
        dema_long = DEMAIndicator(
            close, window=50
        ).dema_indicator()
        ...
        avg_quote_vol_20 = (
            price_data["quote_volume"].iloc[-20:].mean()
            if "quote_volume" in price_data
            else None
        )
        latest_quote_vol = (
            price_data["quote_volume"].iloc[-1]
            if "quote_volume" in price_data
            else None
        )
        ...
        if avg_quote_vol_20 is None:
            avg_quote_vol_20 = (
                volume.iloc[-20:].mean() * price_now
            )
            latest_quote_vol = (
                volume.iloc[-1] * price_now
            )
        ...
        vol_factor = session_factor.get(
            session_name, 0.4
        )
        vol_threshold = max(
            vol_factor * avg_quote_vol_20, 50_000
        )
        if latest_quote_vol < vol_threshold:
            print(
                f"⛔ Skipping due to low volume: {latest_quote_vol:,.0f} < {vol_threshold:,.0f} ({vol_factor*100:.0f}% of 20-bar avg)"
            )
            return 0, None, 0, None
        ...
        reinforcement = (
            1.0
        )  # placeholder; pattern memory bonus is applied later
        score = 0.0
        ...
        vwma_dev = (
            abs(price_now - vwma_value) / price_now
            if price_now != 0
            else 0.0
        )
        ...
        if (
            bb_lower.iloc[-1]
            and bb_lower.iloc[-1] < price_now < bb_upper.iloc[-1]
        ):
            score += w["bb"]
        ...
        normalized_score = round(
            (score / max_possible) * 10 * reinforcement, 2
        )
        # Sentiment bias adjustments
        if (
            sentiment_bias == "bullish"
            and normalized_score < 5.0
        ):
            normalized_score += 0.8
        elif (
            sentiment_bias == "bearish"
            and normalized_score > 7.5
        ):
            normalized_score -= 0.8
        normalized_score = round(normalized_score, 2)
        direction = (
            "long" if normalized_score >= 4.5 else None
        )
        ...
        zones = (
            zones.to_dict()
            if isinstance(zones, pd.Series)
            else (zones or {"support": [], "resistance": []})
        )
        ...
        if direction == "long":
            ...
            if (
                near_resistance
                and normalized_score < 7.0
            ):
                return 0, None, 0, None
            if (
                not near_support
                and normalized_score < 6.5
            ):
                return 0, None, 0, None
        ...
        try:
            ...
            with open(
                SYMBOL_SCORES_FILE, "w"
            ) as f:
                json.dump(scores, f, indent=2)
        except Exception as e:
            print(
                f"[SYMBOL SCORES] Failed to update symbol_scores.json: {e}"
            )
        return (
            normalized_score,
            direction,
        ...
        print(
            f"⚠️ Signal evaluation error in {symbol}: {e}"
        )
        return 0, None, 0, None
